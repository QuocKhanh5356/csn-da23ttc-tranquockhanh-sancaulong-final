...
if (!isset($_GET['id'])) {
    header("Location: dashboard_admin.php?pg=employeeManage");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $name = $_POST['name'];
    $email = $_POST['email'];
    $password = !empty($_POST['password']) ? md5($_POST['password']) : $employee['password'];
    $phone = $_POST['phone'];
    $address = $_POST['address'];

    if ($email != $employee['email'] && $employeeService->isEmailExist($email)) {
        $error = "Email đã tồn tại!";
    } elseif (!$employeeService->validatePhoneNumber($phone)) {
        $error = "Số điện thoại không hợp lệ!";
    } else {
        $result = $employeeService->updateEmployee($id, $name, $password, $phone, $email, $address);
        if ($result) {
            echo '<script type="text/javascript">alert("Nhân viên đã được cập nhật thành công."); location.replace("dashboard_admin.php?pg=employeeManage");</script>';
            exit();
        } else {
...
<body>
    <div class="container mt-5">
        ...
        <form method="POST" action="">
            ...
            <div class="mb-3">
                <label for="password" class="form-label"><i class="fas fa-lock"></i> Mật khẩu (để trống nếu không đổi)</label>
